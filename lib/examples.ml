...
let body = 
    let body' = 
      let loop = expr (Ewhile (expr Etrue, (* while true *)
        ...
        expr (Eif (eapp gt [eapp bng [mk_Evar "i"]; mk_Evar "m"], expr (Eoptexn (ident "return", MaskVisible ,expr Etrue)), (* if i > m then return true *)
        expr (Eif (eapp eq_int [eapp md [mk_Evar "n"; eapp bng [mk_Evar "i"]]; econst 0], expr (Eoptexn (ident "return", MaskVisible ,expr Efalse)), (* else if n%i = 0 then return false *)
